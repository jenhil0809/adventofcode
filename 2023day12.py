...
with open("2023day12.txt", "r") as file:
    puzzle_input = [line for line in file]


@cache
def ways(sequence, groups):
    if not groups:
    ...
    if sequence[0] == '#':
        return ways(sequence[group_len + 1:].lstrip('.'), tuple(groups[1:])) if can_use else 0
    skip = ways(sequence[1:].lstrip('.'), groups)
    if not can_use:
        return skip
    return skip + ways(sequence[group_len + 1:].lstrip('.'), tuple(groups[1:]))
...
for line in puzzle_input:
    ...
    total += ways(sequence, tuple(groups))
